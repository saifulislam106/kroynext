...
export class CategoryService {
  ...
  async update(id: string, updateCategoryDto: UpdateCategoryDto): Promise<CategoryResponseDto> {
    ...
    if(updateCategoryDto.slug && updateCategoryDto.slug !){
    ...
    return this.formatCategory(updateCategory, Number(updateCategory._count.products));
  }
  ...
  async remove(id: string) {
    ...
    if(existingCategory._count.products >0){
      throw new BadRequestException(`Cannot delete category with ${existingCategory._count.products} products. Remove or reassign first`,
)
    }
